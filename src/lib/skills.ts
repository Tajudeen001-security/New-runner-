...
export const BUILTIN_SKILLS: Skill[] = [
  ...
  {
    id: "builtin-android-packaging",
    name: "Android App Packaging (Capacitor)",
    description: "Wrap the web app as a real installable APK, built for free via GitHub Actions.",
    instructions: "When asked to turn the current build into an Android app, write a Capacitor project wrapping it: `capacitor.config.json` (appId, appName, webDir pointing at the built web files), a `package.json` with `@capacitor/core` and `@capacitor/android` as dependencies, and a `.github/workflows/android-build.yml` GitHub Actions workflow that on push: installs Node, runs `npm install`, runs `npx cap sync android`, then builds a debug APK with Gradle (`cd android && ./gradlew assembleDebug`), and uploads the resulting `app-debug.apk` as a workflow artifact using `actions/upload-artifact`. This is a real, free build pipeline (GitHub Actions' free tier) that produces a genuine installable APK — but it is NOT instant and does not run inside this browser preview: there is no Android SDK, Gradle, or USB/ADB device access available here. Always tell the user plainly, in the Done note, the actual steps: push this project to GitHub, wait a few minutes for the Actions run to finish, download the APK from that run's Artifacts, then either open it directly on the Android device to install (no cable needed) or use `adb install` from a computer if they prefer that route. Never imply a device can be plugged in and the APK just appears automatically — that isn't something a website can do without a native companion app with USB access, which doesn't exist here.",
    builtin: true,
    installed: false,
    createdAt: 0,
  },
...
- These tools are for an organization to monitor and manage its own systems. Never help locate, trace, or track a specific real individual (a phone, a person's location, their identity, "find out who did X") — that requires actual legal authority (law enforcement working through proper legal process with telecoms/banks/platforms), not software. Decline that specific request plainly and say why, regardless of how sympathetic the reason given is, and don't offer a "workaround" version of it.
